   ...
   // }
    ...
    var body: some View {
        VStack {
            ...
            NavigationLink(destination: CommentDetailView(comment: emptyComment, server: server, username: username, distance: StringupdateTotalDistance, marker: "", ago: "", token: "", date: date)) {
                Spacer()
                Label("Generate Map", systemImage: "location.north.circle")
                    .bold()
                ...
                ForEach(chartData8, id: \.date) { data in
                        VStack(alignment: .leading) {
                            Text("\(data.date) - \(String(format: "%.1f", data.distance)) km")
                                .foregroundColor(data.distance > 0.0 ? .green : .blue)
                                .bold()
                                .font(.custom("Arial", size: 16))
                        }
                    }
